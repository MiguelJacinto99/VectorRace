...
class Graph:
    ...
    def greedy(self):
        ...
        while checkPointsCruzados <= totalCheckPoints:
            ...
            listaDistCheckPoint = []
            if len(carro.listaMovimentos) > 0:
                ...
                if self.cruzouNodesImportantes(linhaAtual, colunaAtual, carro.linha, carro.coluna, checkPointsCruzados):
                    checkPointsCruzados = checkPointsCruzados + 1
                    print("CheckPoint Cruzado!")
            else:
                t = linhaAtual, colunaAtual, velocidadeLinhaAtual, velocidadeColunaAtual
                linhaAnterior, colunaAnterior, velocidadeLinhaAnterior, \
                velocidadeColunaAnterior = parents[t]
                carro.linha = linhaAnterior
                carro.coluna = colunaAnterior
                carro.velocidadeLinha = velocidadeLinhaAnterior
                carro.velocidadeColuna = velocidadeColunaAnterior
                # Remove o Pai. Caminho Sem Saída.
                parents.pop(t, None)
        # -----------------------------------------------------------------------------------------------------
            ...
            if len(carro.listaMovimentos) > 0:
                for idNode, velocidadeLinha, velocidadeColuna in carro.listaMovimentos:
                    ...
                    if self.cruzouNodesImportantes(linhaAtual, colunaAtual, carro.linha, carro.coluna, -1):
                        path = []
                        while parents[outroT] != outroT:
                            path.append(outroT)
                            outroT = parents[outroT]
                        t = ()
                        t = self.nodes[self.partida].linha, self.nodes[self.partida].coluna, 0, 0
                        path.append(t)
                        path.reverse()
                ...
                # Remove o Pai. Caminho Sem Saída.
                parents.pop(t, None)
                if carro.linha == self.nodes[self.partida].linha and \
                    carro.coluna == self.nodes[self.partida].coluna:
                    existeCaminho = False
        return None